...
;(function(run) {
  ...
  run(nav, w, lang, exports);

})(function(nav, w, lang, exports) {
  ...
  var ed = exports.editor = {
    ...
    rewrite: function(ast, nodeToReplace, newNodes) {
      ...
      var replaced = parents.reduceRight(function(replacement, parent) {
        ...
        var newParent = lang.obj.merge(parent, {
    ...
    splitSexp: function(ast, idx) {
      ...
      var sexp = sexps.pop();
      if (!w.hasChildren(sexp)) return null; // later
    ...
    wrapAround: function(ast, idx, wrapWithStart, wrapWithEnd, howMany) {
      howMany = typeof howMany === "number" ? howMany : 1;
      var sexps = w.containingSexpsAt(ast,idx);
      if (!sexps.length) return ast;
      var parent = last(sexps);
      var sexpsToWrap = parent.children.filter(function(c) {
        return c.start >= idx; }).slice(0,howMany);
      var end = last(sexpsToWrap);
      console.log(sexpsToWrap);
      var changes = [
      ...
      return linesToIndent.reduce(function(indent, line) {
        ...
        if (right) {
          ...
          ast = ed.rewrite(ast, parent,
            [lang.obj.merge(parent, {end: parent.end+indentOffset})]);
        }
  ...
  function moveNode(offset, n) {
    ...
    return lang.tree.mapTree(n,
      function(n, children) {
        return lang.obj.merge(n, {

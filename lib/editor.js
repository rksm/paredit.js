...
// var src = "(foo\nbar baz)"
      ...
      var toIndent = linesToIndent.reduce(function(indent, line) {
        ...
        if (true || indentOffset !== 0) {
          
          changes.push(indentOffset >= 0 ?
            ["insert", idx, times(indentOffset, " ")] :
            ["remove", idx, Math.abs(indentOffset)]);
          lineLength += indentOffset;
          // "move" the next node to the right accordingly, "update" the entire ast
      ...
      }, {idx: startLineIdx, changes: [], ast: ast});

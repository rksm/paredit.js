  ...
  var ed = exports.editor = {
    ...
    delete: function(ast,src,idx,args) {
      ...
      if (left && left.length && backward) {
        var n = last(left);
        if (n.end !== idx || n.type === 'symbol'|| n.type === 'comment' || n.type === 'number') return simpleDelete;
      ...
      if (right && right.length && !backward) {
        var n = right[0];
        if (n.start !== idx || n.type === 'symbol'|| n.type === 'comment' || n.type === 'number') return simpleDelete;
        if (isEmpty(n) || n.type === "char") return deleteSexp(n);
        return noDelete;
      }
